...
const strength = computed(() => {
  ...
  const map = [
    ...
    { level: 2, label: "Fair",    color: "#e8a838" },
    { level: 3, label: "Good",    color: "#60a5fa" },
  ...
  <div class="page">
    ...
    <div class="card" role="main">
      ...
      <div class="card__head">
        ...
        <p class="card__sub">Set up your workspace in seconds</p>
      </div>
      ...
      <form class="card__form" @submit.prevent="submit" novalidate>
        ...
        <div>
          <AppInput
            ...
            :hint="strength.label ? `Strength: ${strength.label}` : ''"
          />
          ...
          <div class="strength" v-if="form.password">
            <div
              ...
              :style="{
                ...
                transition: `background 0.2s ${(i-1) * 0.05}s`
              }"
            />
          </div>
        </div>
      ...
      <p class="card__footer">
        Have an account?
        <router-link to="/login" class="card__link">Sign in</router-link>
      </p>
    </div>
  </div>
</template>
...
.page {
  ...
  justify-content: center; background: var(--main);
...
.card__brand { display: flex; align-items: center; gap: 8px; font-size: 10px; font-weight: 700; letter-spacing: 0.15em; color: var(--folder-color); margin-bottom: 28px; }
...
.strength__bar { flex: 1; height: 3px; border-radius: 2px; }
</style>